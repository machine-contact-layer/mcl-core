...
| 18 | Full robustness gates | `DONE` | Both compiler halves are green on the release candidate; the executable receipts report the exact POSIX and MSVC counts. Both scripts count what actually ran; take the number from the run, not from this row. Re-run at the release candidate per `GOVERNANCE.md` §7. |
...
| 31 | Reconstructability check | `DONE` | Bundle reconstruction passed in the 2026-09-12 final rehearsal, including extracted SDK build and contract test: `RECONSTRUCTION VERIFIED`, exit 0. See `conformance/independent/20260912-private-final/reconstruction.log`. **This row previously cited the 2026-09-10 rehearsal.** That receipt was not withdrawn and is not edited -- it stopped being the *final* evidence when executable source changed after it (the Base 1 send path and the arranged-bearer deployment), and a row citing a rehearsal that predates the code it qualifies is a ledger disagreeing with the tree. |
| 32 | Final clean-room release rehearsal | `DONE` | 2026-09-12 POSIX rehearsal: **22 checks passed, 0 failed**, exit 0; MSVC: **44 test targets passed**, exit 0. Receipts in `conformance/independent/20260912-private-final/`, with the eight revisions they ran against in `HEADS.json` and all eight working trees clean before and after. The count moved from 20/43 because `check-base-deployment.sh`, `check-evidence-digests.sh` and the `test_base1` target were added after the earlier rehearsal. **The first attempt failed seven checks and the tree was not at fault**: the 2026-09-10 rehearsal had run as root and left root-owned working directories in `/tmp`, so an unprivileged re-run could not rebuild them and the go/no-go audit inherited two FATALs from the two it could not regenerate -- read quickly, an API baseline break and a stale specification index; in fact one-to-one with the leftovers. These receipts were taken after clearing them, unprivileged, with a private empty `TMPDIR`, which is a stronger configuration than the run it replaces. Source revisions are frozen in the bundle; later closure metadata changes no executable source. |
| 33 | Final go/no-go audit | `DONE` | 2026-09-12 go/no-go: **no fatal findings**, exit 0; retained informational hits reviewed as historical evidence, reserved vocabulary and Experimental Use identifiers. See `conformance/independent/20260912-private-final/go-no-go.log`. Public visibility and external review remain open. |
