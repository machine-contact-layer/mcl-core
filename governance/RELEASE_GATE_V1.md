...
```text
35       builder-interoperability floor                 ACTIVE
30-33    final bundle, reconstruction, rehearsal, audit WAIT_DEP
27       publication and evidence disclosure            EXTERNAL
36       charter-required public review                 EXTERNAL
34       tag v1.0.0 after the preceding gates           WAIT_DEP
```
...
asynchronous candidate refusal. **That Android adapter has now been rebuilt,
installed and run against the DFR1154.** The first zero-prior attempt exposed
lost-acceptance recovery in the board adapter. Two corrected attempts crossed
acoustic bootstrap in both directions and matched the exact air-learned Android
BLE advertisement, then failed when the DFR central could not establish the
connection. These retained negative runs advance the boundary but do not
replace either physical Row 35 success criterion.
