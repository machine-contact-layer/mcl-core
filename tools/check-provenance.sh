...
# sit under a directory whose README says what produced it. A binary anywhere
# else entered without a decision.
#
# THREE ACCOUNTED LOCATIONS, NOT TWO
#
# evidence/ and experiments/ hold measurements. conformance/vectors/ holds
# something different and equally legitimate: PCM test vectors, GENERATED by a
# tracked tool, which two implementations are compared against. A specification
# that requires a receiver to decode particular bytes cannot ship without those
# bytes, and they cannot be text.
#
# The accounting requirement is unchanged and is the point: every one of these
# still needs a README in its directory tree saying what produced it, and
# conformance/vectors/VECTORS.md is generated by make_vectors.c alongside them.
echo
echo "-- every tracked binary sits in an accounted location, with a README"
unaccounted=0
for repo in $REPOS; do
...
[ "$unaccounted" -eq 0 ] && echo "  ok   $total_binaries binary artifact(s), all in an accounted location with a README"
